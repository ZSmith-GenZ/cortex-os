...
(async () => {
  ...
  if (docker) {
    ...
    console.purple('Pruning all Cortex OS Docker images...');

    const imageName = singleCompose ? 'cortex-os_single' : 'cortex-os';
    try {
      execSync(`${sudo}docker rmi ${imageName}:latest`, { stdio: 'inherit' });
    } catch (e) {
      console.purple('Failed to remove Docker image cortex-os:latest. It might not exist.');
    }
    console.purple('Removing all unused dangling Docker images...');
    execSync(`${sudo}docker image prune -f`, { stdio: 'inherit' });
    console.purple('Building new Cortex OS image...');
  ...
  console.green('Your Cortex OS app is now up to date! Start the app with the following command:');
  console.purple(startCommand);
  console.orange(
    "Note: it's also recommended to clear your browser cookies and localStorage for Cortex OS to assure a fully clean installation.",
  );
