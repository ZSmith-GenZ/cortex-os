...
const { genAzureEndpoint, logAxiosError } = require('@cortex-os/api');
const { extractEnvVariable, TTSProviders } = require('@cortex-os/data-provider');
...
class TTSService {
  ...
  getProvider(appConfig) {
    ...
    if (!ttsSchema) {
      throw new Error(
        'No TTS schema is set. Did you configure TTS in the custom config (cortex-os.yaml)?',
      );
    }
