...
const { EModelEndpoint, Constants, openAISettings, CacheKeys } = require('@cortex-os/data-provider');
...
/**
 * Imports a Cortex OS conversation from JSON.
 *
...
async function importCortexOSConvo(
  ...
  try {
    ...
    if (jsonData.recursive) {
      ...
      throw new Error('Invalid Cortex OS file format');
    }
    ...
    logger.error(`user: ${requestUserId} | Error creating conversation from Cortex OS file`, error);
  }
}
