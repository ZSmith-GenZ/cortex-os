# DNS Configuration for Cortex OS Helm Chart

This feature allows you to configure custom DNS settings for Cortex OS pods, enabling traffic redirection to proxy servers or custom endpoints.
...
Deploy:
```bash
helm upgrade --install cortex-os ./helm/cortex-os -f values.yaml
```
...
```bash
helm install cortex-os ./helm/cortex-os \
  --set dnsPolicy="None" \
...
✅ **Integration Test**
- Works with existing Cortex OS configuration
...
- Kubernetes 1.19+
- Compatible with all Cortex OS deployment modes
