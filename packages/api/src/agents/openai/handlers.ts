 ...
 *
 * These handlers convert Cortex OS's internal graph events into OpenAI-compatible
...
/**
 * Step types from @cortex-os/data-provider
 */
...
export class OpenAIRunStepHandler implements EventHandler {
  ...
  handle(_event: string, data: { stepDetails?: { type?: string } }): void {
    // Run step events are primarily for Cortex OS UI, we use deltas for streaming
