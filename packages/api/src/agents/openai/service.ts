 ...
 * interacting with Cortex OS agents. The agent_id is passed as the "model"
 * parameter per OpenAI spec.
 *
 * Usage:
 * ```typescript
 * import { createAgentChatCompletion } from '@cortex-os/api';
 *
...
/**
 * Agent type from @cortex-os/data-provider
 */
interface Agent {
  id: string;
  name?: string;
...
/**
 * Convert OpenAI messages to Cortex OS format
 */
