import { logger, webSearchKeys } from '@cortex-os/data-schemas';
import { Constants, extractVariableName } from '@cortex-os/data-provider';
...
const deprecatedVariables = [
  {
    key: 'CHECK_BALANCE',
    description:
      'Please use the `balance` field in the `cortex-os.yaml` config file instead.\nMore info: https://cortexos.ai/docs/configuration/cortex-os_yaml/object_structure/balance#overview',
  },
  {
    key: 'START_BALANCE',
    description:
...
function checkPasswordReset() {
  ...
  if (!emailEnabled && passwordResetAllowed) {
    logger.warn(
      ...
      https://www.cortexos.ai/docs/configuration/authentication/email
...
export function checkVariables() {
  ...
  if (hasDefaultSecrets) {
    ...
    https://www.cortexos.ai/toolkit/creds_generator
...
Latest version: ${Constants.CONFIG_VERSION}
      ...
      https://www.cortexos.ai/changelog\n\n`,
    );
  }
}
    ...
    if (typeof value === 'string') {
      ...
      if (varName) {
        ...
        logger.warn(
          ...
          This is incorrect! You should use environment variable references in your cortex-os.yaml file, such as:
          ${key}: "\${YOUR_ENV_VAR_NAME}"
          
          Then set the actual API key in your .env file or environment variables.
          
          More info: https://www.cortexos.ai/docs/configuration/cortex-os_yaml/web_search`,
        );
      }
    }
  });
