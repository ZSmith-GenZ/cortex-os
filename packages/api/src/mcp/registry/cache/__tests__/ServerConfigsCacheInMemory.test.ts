...
describe('ServerConfigsCacheInMemory Integration Tests', () => {
  ...
  describe('credential placeholders in YAML configs', () => {
    it('should preserve CORTEX_OPENID placeholders (admin configs are trusted)', async () => {
      ...
      expect(retrievedWithHeaders?.headers?.Authorization).toBe(
        'Bearer {{CORTEX_OPENID_ACCESS_TOKEN}}',
      );
      expect(retrievedWithHeaders?.headers?.['X-User-Id']).toBe('{{CORTEX_OPENID_USER_ID}}');
    });

    it('should preserve CORTEX_USER placeholders (admin configs are trusted)', async () => {
      const adminConfig: ParsedServerConfig & { headers?: Record<string, string> } = {
        type: 'sse',
      ...
      expect(retrievedWithHeaders?.headers?.['X-User-Email']).toBe('{{CORTEX_USER_EMAIL}}');
      expect(retrievedWithHeaders?.headers?.['X-User-Name']).toBe('{{CORTEX_USER_NAME}}');
      expect(retrievedWithHeaders?.headers?.['X-User-Id']).toBe('{{CORTEX_USER_ID}}');
    });
  });
});
