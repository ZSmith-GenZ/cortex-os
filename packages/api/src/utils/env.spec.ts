import { TokenExchangeMethodEnum } from '@cortex-os/data-provider';
import {
  resolveHeaders,
  resolveNestedObject,
  processMCPEnv,
...
import type { MCPOptions } from '@cortex-os/data-provider';
...
describe('resolveHeaders', () => {
  ...
  it('should process full user object placeholders', () => {
    ...
    const headers = {
      ...
      'User-Name': '{{CORTEX_USER_NAME}}',
      'User-Username': '{{CORTEX_USER_USERNAME}}',
  ...
  it('should handle missing user fields gracefully', () => {
    ...
    const headers = {
      'User-Email': '{{CORTEX_USER_EMAIL}}',
      'User-Username': '{{CORTEX_USER_USERNAME}}',
      'Non-Existent': '{{CORTEX_USER_NONEXISTENT}}',
    };
  ...
  it('should prioritize custom user variables over user fields', () => {
    ...
    const customUserVars = {
      CORTEX_USER_EMAIL: 'custom-email@example.com',
    };

    const headers = {
      'Test-Email': '{{CORTEX_USER_EMAIL}}',
    };
  ...
  it('should replace all allowed user field placeholders', () => {
    ...
    const headers = {
      'X-User-ID': '{{CORTEX_USER_ID}}',
      'X-User-Name': '{{CORTEX_USER_NAME}}',
      'X-User-Username': '{{CORTEX_USER_USERNAME}}',
      'X-User-Email': '{{CORTEX_USER_EMAIL}}',
      'X-User-Provider': '{{CORTEX_USER_PROVIDER}}',
      'X-User-Role': '{{CORTEX_USER_ROLE}}',
      'X-User-GoogleId': '{{CORTEX_USER_GOOGLEID}}',
      'X-User-FacebookId': '{{CORTEX_USER_FACEBOOKID}}',
      'X-User-OpenIdId': '{{CORTEX_USER_OPENIDID}}',
      'X-User-SamlId': '{{CORTEX_USER_SAMLID}}',
      'X-User-LdapId': '{{CORTEX_USER_LDAPID}}',
      'X-User-GithubId': '{{CORTEX_USER_GITHUBID}}',
      'X-User-DiscordId': '{{CORTEX_USER_DISCORDID}}',
      'X-User-AppleId': '{{CORTEX_USER_APPLEID}}',
      'X-User-EmailVerified': '{{CORTEX_USER_EMAILVERIFIED}}',
      'X-User-TwoFactorEnabled': '{{CORTEX_USER_TWOFACTORENABLED}}',
      'X-User-TermsAccepted': '{{CORTEX_USER_TERMSACCEPTED}}',
    };
  ...
  it('should handle multiple placeholders in one value', () => {
    ...
    const headers = {
      'X-Multi': 'User: {{CORTEX_USER_ID}}, Env: ${TEST_API_KEY}, Custom: {{MY_CUSTOM}}',
    };
  ...
  it('should handle a mix of all types', () => {
    ...
    const headers = {
      'X-User': '{{CORTEX_USER_ID}}',
      'X-Env': '${TEST_API_KEY}',
      'X-Custom': '{{MY_CUSTOM}}',
      'X-Multi': 'ID: {{CORTEX_USER_ID}}, ENV: ${TEST_API_KEY}, CUSTOM: {{MY_CUSTOM}}',
      'X-Unknown': '{{NOT_A_REAL_PLACEHOLDER}}',
      'X-Empty': '',
      'X-Boolean': '{{CORTEX_USER_EMAILVERIFIED}}',
    };
    const customVars = { MY_CUSTOM: 'custom-value' };
    const result = resolveHeaders({ headers, user, customUserVars: customVars });
  ...
  it('should process CORTEX_BODY placeholders', () => {
    const body = {
      conversationId: 'conv-123',
      parentMessageId: 'parent-456',
      messageId: 'msg-789',
    };
    const headers = { 'X-Conversation': '{{CORTEX_BODY_CONVERSATIONID}}' };
  ...
  describe('non-string header values (type guard tests)', () => {
    ...
    it('should handle numeric values with placeholders', () => {
      ...
      const headers = {
        ...
        'X-String-With-Placeholder': '{{CORTEX_USER_ID}}',
      };
      const result = resolveHeaders({ headers, user });
    ...
    it('should handle numeric values with body placeholders', () => {
      ...
      const headers = {
        ...
        'X-Conv': '{{CORTEX_BODY_CONVERSATIONID}}',
      };
      const result = resolveHeaders({ headers, body });
    ...
    it('should not crash when calling includes on non-string body field values', () => {
      ...
      const headers = {
        'X-Conv-Id': '{{CORTEX_BODY_CONVERSATIONID}}',
        'X-Number': 999 as unknown as string,
      };
...
describe('resolveNestedObject', () => {
  ...
  it('should process strings in arrays', () => {
    const user = { id: 'user-123' };
    const obj = {
      headers: ['Authorization: Bearer ${TEST_API_KEY}', 'X-User-Id: {{CORTEX_USER_ID}}'],
      values: [1, 2, 3],
      mixed: ['string', 42, true, '{{CORTEX_USER_ID}}'],
    };
  ...
  it('should handle body placeholders in nested objects', () => {
    ...
    const obj = {
      metadata: {
        conversation: '{{CORTEX_BODY_CONVERSATIONID}}',
        parent: '{{CORTEX_BODY_PARENTMESSAGEID}}',
        count: 5,
      },
    };
  ...
  it('should handle arrays of objects', () => {
    ...
    const obj = {
      items: [
        { name: 'item1', user: '{{CORTEX_USER_ID}}', count: 1 },
        { name: 'item2', user: '{{CORTEX_USER_ID}}', count: 2 },
      ],
    };
  ...
  it('should not modify the original object', () => {
    ...
    expect(originalObj.thinking.user_id).toBe('{{CORTEX_USER_ID}}');
  });
});
...
describe('processMCPEnv', () => {
  ...
  it('should process user field placeholders in all fields', () => {
    ...
    const options: MCPOptions = {
      ...
      env: {
        ...
        USER_ROLE: '{{CORTEX_USER_ROLE}}',
      },
      args: ['--user', '{{CORTEX_USER_USERNAME}}', '--id', '{{CORTEX_USER_ID}}'],
    };
  ...
  it('should process body placeholders in all fields', () => {
    ...
    const options: MCPOptions = {
      type: 'streamable-http',
      url: 'https://api.example.com/conversations/{{CORTEX_BODY_CONVERSATIONID}}',
      headers: {
        'X-Parent-Message': '{{CORTEX_BODY_PARENTMESSAGEID}}',
        'X-Message-Id': '{{CORTEX_BODY_MESSAGEID}}',
      },
    };
  ...
  it('should handle mixed placeholders in OAuth configuration', () => {
    ...
    const options: MCPOptions = {
      ...
      oauth: {
        ...
        client_id: '${OAUTH_CLIENT_ID}',
        client_secret: '${OAUTH_CLIENT_SECRET}',
        scope: 'user:{{CORTEX_USER_ID}} conversation:{{CORTEX_BODY_CONVERSATIONID}}',
        redirect_uri: 'http://localhost:3000/user/{{CORTEX_USER_EMAIL}}/callback',
      },
    };
  ...
  it('should handle all placeholder types in a single value', () => {
    ...
    const options: MCPOptions = {
      ...
      env: {
        COMPLEX_VALUE:
          'User: {{CORTEX_USER_ID}}, Custom: {{CUSTOM_VAR}}, Body: {{CORTEX_BODY_CONVERSATIONID}}, Env: ${TEST_API_KEY}',
      },
    };
  ...
  describe('non-string values (type guard tests)', () => {
    ...
    it('should not crash with numeric body field values', () => {
      ...
      const options: MCPOptions = {
        ...
        args: [],
        env: {
          CONV_ID: '{{CORTEX_BODY_CONVERSATIONID}}',
          PORT: 8080 as unknown as string,
        },
      };
