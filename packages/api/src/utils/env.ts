import { extractEnvVariable } from '@cortex-os/data-provider';
 ...
 *
 * NOTE: This is a Cortex OS-specific encoding scheme to work around Fetch API limitations.
...
function processUserPlaceholders(
  ...
  for (const field of ALLOWED_USER_FIELDS) {
    const placeholder = `{{CORTEX_USER_${field.toUpperCase()}}}`;
 ...
 * Recognized placeholders: `{{CORTEX_BODY_<FIELD>}}` where `<FIELD>` ∈ ALLOWED_BODY_FIELDS.
...
function processBodyPlaceholders(value: string, body: RequestBody): string {
  ...
  for (const field of ALLOWED_BODY_FIELDS) {
    const placeholder = `{{CORTEX_BODY_${field.toUpperCase()}}}`;
