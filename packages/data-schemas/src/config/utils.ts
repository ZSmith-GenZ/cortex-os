 ...
 * Priority:
 * 1. CORTEX_LOG_DIR environment variable
 * 2. If running within Cortex OS monorepo (when cwd ends with /api), use api/logs
...
export const getLogDirectory = (): string => {
  if (process.env.CORTEX_LOG_DIR) {
    return process.env.CORTEX_LOG_DIR;
  }
  ...
  // For Cortex OS project structure, use api/logs
  // For external consumers, they should set CORTEX_LOG_DIR
  if (cwd.includes('Cortex OS')) {
    return apiLogsPath;
  }
